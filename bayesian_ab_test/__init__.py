...
# define function for testing counts
def bayesian_ab_test_count(sample_A_n, sample_A_count, sample_B_n, sample_B_count, N_simulations=1000, pct_tune=50, gr_threshold=1.01, N_additional_draws=1000, lpv_height=15, n_x_observed=2):
    ...
    N_nonclicks_A = sample_A_n - sample_A_count
    # calculate CTR
    observed_p_A = sample_A_count/sample_A_n
    # calculate lower plausible value in sample A
    sample_A_lpv = plausible_values(total=sample_A_n, upvotes=sample_A_count)[0]
    # calculate upper plausible value in sample A
    sample_A_upv = plausible_values(total=sample_A_n, upvotes=sample_A_count)[1]
    # sample B
    # calculate non-clicks
    N_nonclicks_B = sample_B_n - sample_B_count
    # calculate CTR
    observed_p_B = sample_B_count/sample_B_n
    # calculate lower plausible value in sample B
    sample_B_lpv = plausible_values(total=sample_B_n, upvotes=sample_B_count)[0]
    # calculate upper plausible value in sample B
    sample_B_upv = plausible_values(total=sample_B_n, upvotes=sample_B_count)[1]
    
    # put into df
    df = pd.DataFrame({'Variable': ['Total','Yes','No','Rate','LPV','UPV'],
                       'Sample A': [sample_A_n, sample_A_count, N_nonclicks_A, observed_p_A, sample_A_lpv, sample_A_upv],
                       'Sample B': [sample_B_n, sample_B_count, N_nonclicks_B, observed_p_B, sample_B_lpv, sample_B_upv]})
    ...
    with pm.Model() as model:
        ...
        prior_A_count = pm.Uniform('prior_A_count', 0, n_x_observed*sample_A_count)
        prior_B_count = pm.Uniform('prior_B_count', 0, n_x_observed*sample_B_count)
        prior_A_n = pm.Uniform('prior_A_n', 0, n_x_observed*sample_A_n)
        prior_B_n = pm.Uniform('prior_B_n', 0, n_x_observed*sample_B_n)
        # get posterior probabilities
        post_A_count = pm.Poisson('post_A_count', prior_A_count, observed=sample_A_count)
        post_B_count = pm.Poisson('post_B_count', prior_B_count, observed=sample_B_count)
        post_A_n = pm.Poisson('post_A_n', prior_A_n, observed=sample_A_n)
        post_B_n = pm.Poisson('post_B_n', prior_B_n, observed=sample_B_n)
    ...
    # get distributions
    # calculate count/n
    prior_A_count = trace['prior_A_count']
    prior_B_count = trace['prior_B_count']
    prior_A_n = trace['prior_A_n']
    prior_B_n = trace['prior_B_n']
    
    # instantiate empty lists
    sample_A_count_div_n_list = []
    sample_B_count_div_n_list = []
    for i in range(0, len(prior_A_count)):
        # calculate count/n for A
        sample_A_count_div_n = prior_A_count[i]/prior_A_n[i]
        sample_A_count_div_n_list.append(sample_A_count_div_n)
        # calculate count/n for B
        sample_B_count_div_n = prior_B_count[i]/prior_B_n[i]
        sample_B_count_div_n_list.append(sample_B_count_div_n)
    ...
    # posterior A
    p1 = sns.distplot(sample_A_count_div_n_list, color='b', label='Posterior of $p_A$')
    p1.vlines(sample_A_lpv, 0, lpv_height, colors='b', linestyle='--', label='Sample A LPV: {0:0.3f}'.format(sample_A_lpv))
    p1.legend(loc='upper left')
    # posterior B
    p2 = sns.distplot(sample_B_count_div_n_list, color='r', label='Posterior of $p_B$')
    p2.vlines(sample_B_lpv, 0, lpv_height, colors='r', linestyle='--', label='Sample B LPV: {0:0.3f}'.format(sample_B_lpv))
    ...
    for i in range(len(sample_A_count_div_n_list)):
        if sample_A_count_div_n_list[i] > sample_B_count_div_n_list[i]:
            sum_of_A_greater_than_B += 1
        
    # calculate proportion A greater than B    
    proportion_A_greater_than_B = sum_of_A_greater_than_B/len(sample_A_count_div_n_list)
    ...
    class Attributes:
        def __init__(self, df, lpv_plot, sample_A_count_div_n_list, sample_B_count_div_n_list, bfmi, max_gr, dist_plot, proportion_A_greater_than_B, proportion_B_greater_than_A):
            self.df = df
            self.lpv_plot = lpv_plot
            self.sample_A_count_div_n_list = sample_A_count_div_n_list
            self.sample_B_count_div_n_list = sample_B_count_div_n_list
            self.bfmi = bfmi
            self.max_gr = max_gr
    ...
    x = Attributes(df, lpv_plot, sample_A_count_div_n_list, sample_B_count_div_n_list, bfmi, max_gr, dist_plot, proportion_A_greater_than_B, proportion_B_greater_than_A)
    return x

###############################################################################
###############################################################################

# define function for testing probabilities
def bayesian_ab_test_prob(sample_a_total, sample_a_responses, sample_b_total, sample_b_responses, N_simulations=1000, pct_tune=50, gr_threshold=1.001, N_additional_draws=1000, lpv_height=30):
    ###########################################################################
    # get parameters for model
    # make pct_tune into a proportion
    prop_tune = pct_tune/100
